...
Skills estendem o Claude Code com instruções reutilizáveis via arquivo `SKILL.md`. O Claude pode invocá-las automaticamente ou você usa `/skill-name`. Suportam frontmatter rico (disable-model-invocation, context fork, allowed-tools, hooks), arquivos de suporte, injeção dinâmica de contexto com `` !`command` `` e execução em subagente isolado. Seguem o padrão aberto Agent Skills (agentskills.io).
