...
Hooks são comandos shell executados automaticamente em pontos específicos do ciclo de vida do Claude Code (PreToolUse, PostToolUse, Stop, etc.). Permitem controle determinístico: formatar código ao salvar, enviar notificações, bloquear edições, reinjetar contexto e auditar configurações. Suportam quatro tipos de handler: command (shell), http (webhook), prompt (avaliação LLM) e agent (subagente).
