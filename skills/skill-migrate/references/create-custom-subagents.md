...
Subagentes são assistentes especializados que rodam em janela de contexto própria, com system prompt customizado, ferramentas específicas e permissões independentes. Use para tarefas que poluiriam o contexto principal (logs, busca em arquivos, pesquisa). Crie via `/agents`, configure scope (user/project), ferramentas, modo de permissão e hooks. Built-ins: Explore (somente leitura), Plan (planejamento), general-purpose.
