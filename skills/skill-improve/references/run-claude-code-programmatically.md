...
Use `claude -p "prompt"` para rodar o Claude Code de forma não interativa (CLI, CI/CD, scripts). `--bare` pula auto-discovery de hooks/skills/MCP para resultados consistentes entre máquinas. Suporta `--output-format json/stream-json`, `--allowedTools`, `--continue`/`--resume`, `--append-system-prompt` e `--json-schema` para saída estruturada. SDK Python/TypeScript disponível para controle programático completo.
